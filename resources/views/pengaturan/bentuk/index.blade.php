    ...
    <section>
        <div class="card">
            <div class="card-header">
                <h4>Form Bentuk Arsip</h4>
            </div>

            <form action="{{ route('bentuks.store') }}" class="needs-validation" novalidate="" method="POST">
                @csrf
                @method('POST')
                <div class="ml-5 mr-5">
                    <div class="form-group">
                        <label>Nama Bentuk Arsip</label>
                        <input name="nama_bentuk" type="text" class="form-control form-control-sm" required="">
                    ...
                    <div class="card-body">
                        <div class="table-responsive">
                            <table class="table table-striped" id="myTable">
                                ...
                                <tbody>
                                    @foreach ($bentuk as $data_bentuk )
                                    <tr>
                                        <td style="width: 5%" class="text-center">
                                            {{ $loop->iteration }}
                                        </td>
                                        <td style="font-size: 15px">{{ $data_bentuk->nama_bentuk }}</td>
                                        <td>
                                            <button id="edit_bentuk" data-id={{ $data_bentuk->id }} data-nama="{{
                                                $data_bentuk->nama_bentuk }}" class="btn btn-primary float-left"
                                                data-toggle="modal" data-target="#exampleModal">
                                                <i class="fas fa-edit"></i> Edit
                                            </button>

                                            <form class="delete"
                                                action="{{ route('bentuks.destroy',['bentuk'=>$data_bentuk->id]) }}"
                                                method="POST">
                                                @csrf
                                                @method('DELETE')
...
@include('pengaturan.bentuk.modal')
@endsection
...
<script>
    ...
    $(document).on("click","#edit_bentuk",function(){
        var bentuk = $(this).data('id');
        var nama = $(this).data('nama');
        var base = "{{ url('/') }}";
        var link = base+"/bentuks/"+bentuk;

        $("#formEditBentuk").attr('action',link);
        $("#editNamaBentuk").val(nama);
    });
</script>
@endpush
